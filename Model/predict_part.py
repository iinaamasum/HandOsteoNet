...
from .dwconv import DepthwiseSeparableConv
...
class BoneAgeModel(nn.Module):
    def __init__(self, img_size=480):
        ...
        self.stage1 = nn.Sequential(
            DepthwiseSeparableConv(32, 64, stride=1), CBAM(64), nn.MaxPool2d(2, 2)
        )
        self.stage2 = nn.Sequential(
            DepthwiseSeparableConv(64, 128, stride=1), CBAM(128), nn.MaxPool2d(2, 2)
        )
        ...
        self.gender_fc = nn.Sequential(
            nn.Linear(1, 8, bias=True), nn.ReLU(inplace=True), nn.LayerNorm(8)
        )
